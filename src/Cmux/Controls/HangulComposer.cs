...
public sealed class HangulComposer
{
    ...
    // The most recent pre-composed syllable we passed straight through to the
    // PTY (via the >= '가' && <= '힣' branch). When the OS IME emits raw jamo
    // RIGHT AFTER such a syllable — which happens with some Korean IMEs on
    // boundary cases like 저+ㄴ → expected 전 but OS emits 저 then ㄴ —
    // we use this to retroactively extend the syllable: send a backspace and
    // re-emit the combined form. Cleared once the composer enters a steady
    // state that's no longer extendable from the previous PTY write.
    private char? _lastWrittenSyllable;
    ...
    {
        // Already-composed Hangul syllable — pass it through immediately.
        // Stash it as `_lastWrittenSyllable` so that if a stray raw jamo
        // arrives next (some Korean IMEs emit "저" then a separate "ㄴ"
        // instead of composing "전"), we can retroactively combine via
        // backspace + replace.
        if (c is >= '가' and <= '힣')
        {
            // Duplicate-finalize guard: some IMEs emit a partial syllable
            // commit, then raw jamo, then ALSO re-emit the final composed
            // syllable (e.g. "끼", ㅁ raw, "낌"). Our raw-jamo path already
            // produced the final form via "\b낌"; if the OS now hands us
            // the same syllable as a pre-composed finalize, skip the
            // duplicate write — PTY already has it.
            if (_lastWrittenSyllable == c)
            {
                _cho = -1; _jung = -1; _jong = -1;
                return "";
            }

            var flushed = Flush();
            _lastWrittenSyllable = c;
            return flushed + c;
        }
        ...
        {
            var flushed = Flush();
            _lastWrittenSyllable = null;
            return flushed + c;
        }

        // Retroactive extension: a raw jamo arrived right after we passed
        // through a finalized syllable, and the composer has no in-progress
        // state of its own. Fold the jamo into the previous syllable and
        // emit "\b" + combined-syllable so the PTY ends up with the
        // properly composed form (e.g. PTY "...저" + jamo ㄴ → "...전").
        if (_cho < 0 && _lastWrittenSyllable.HasValue)
        {
            var extended = TryExtendLastSyllable(c, choIdx, jungIdx);
            if (extended != null)
                return extended;
        }
    ...
    /// <summary>
    /// Attempts to fold a fresh jamo into the syllable we just emitted to
    /// the PTY (passed through verbatim from the OS IME). Returns
    /// "\b" + new combined syllable on success, or null if the jamo can't
    /// be folded — caller should fall through to normal jamo handling.
    /// </summary>
    private string? TryExtendLastSyllable(char rawJamo, int choIdx, int jungIdx)
    {
        if (!_lastWrittenSyllable.HasValue) return null;
        char prev = _lastWrittenSyllable.Value;
        int offset = prev - 0xAC00;
        int prevCho = offset / (21 * 28);
        int prevJung = (offset / 28) % 21;
        int prevJong = offset % 28;

        // Consonant: try to attach as jong of the previous syllable.
        if (choIdx >= 0 && jungIdx < 0)
        {
            int jongIdx = GetJongIndexFromCho(choIdx);
            if (jongIdx < 0) { _lastWrittenSyllable = null; return null; }
            ...
            else if (CompoundJongMap.TryGetValue((prevJong, jongIdx), out int compound))
            {
                newJong = compound;
            }
            else
            ...
            int code = 0xAC00 + (prevCho * 21 + prevJung) * 28 + newJong;
            char combined = (char)code;
            _cho = prevCho;
            _jung = prevJung;
            _jong = newJong > 0 ? newJong : -1;
            _lastWrittenSyllable = combined;
            return "\b" + combined;
        }

        // Vowel: previous's jong (if any) breaks off and pairs with this
        // vowel as the start of a new syllable. PTY needs to keep the
        // previous (jong-stripped) syllable AND get the new syllable.
        if (jungIdx >= 0 && choIdx < 0)
        {
            if (prevJong == 0) { _lastWrittenSyllable = null; return null; }

            int leftJong;
            int nextCho;
            if (CompoundJongSplit.TryGetValue(prevJong, out var sp))
            {
                leftJong = sp.leftJong;
                nextCho = sp.rightCho;
            }
            else
            {
                int simpleNextCho = JongToChoIndex[prevJong];
                if (simpleNextCho < 0) { _lastWrittenSyllable = null; return null; }
                leftJong = 0;
                nextCho = simpleNextCho;
            }

            int leftCode = 0xAC00 + (prevCho * 21 + prevJung) * 28 + leftJong;
            char left = (char)leftCode;
            _cho = nextCho;
            _jung = jungIdx;
            _jong = -1;
            _lastWrittenSyllable = null;
    ...
    public string Flush()
    {
        // If state already matches what's in the PTY (because we extended
        // a passthrough'd syllable in place), don't re-emit it.
        if (_lastWrittenSyllable.HasValue && _cho >= 0 && _jung >= 0)
        {
            int jongValue = _jong >= 0 ? _jong : 0;
            int code = 0xAC00 + (_cho * 21 + _jung) * 28 + jongValue;
            if ((char)code == _lastWrittenSyllable.Value)
            {
                _cho = -1; _jung = -1; _jong = -1;
                _lastWrittenSyllable = null;
                return "";
